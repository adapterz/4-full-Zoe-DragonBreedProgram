...
### 느낀점
  ...
  기존에 있던 클래스와 코드가 중복되는 클래스를 상속받아 써서 코드 재사용을 막을 수 있어 편리했습니다. 
  
  스레드를 프로그램에 추가한 뒤 보다 현실성을 부여할 수 있어서 프로그램의 완성도가 올라갔다는 생각이 들었습니다.
  

### 개선하고 싶은 점 

  Dragon을 상속받은 각 클래스에 현재 성장단계를 표시해주는 enum 필드를 추가했다면 장단계 별 분기처리를 할 때 더 편리했을 것 같습니다.  
